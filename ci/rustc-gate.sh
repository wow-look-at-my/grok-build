...
# One line per gated invocation. Without it a run cannot say whether the cap saw three compiles or
# sixteen, which is the difference between the cap working and the outer -j oversubscribing the CPU.
if [ -n "${GATE_LOG:-}" ]; then
	printf 'gated\n' >> "$GATE_LOG"
fi

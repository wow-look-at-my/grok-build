...
- **A failed fetch must leave no entry.** `get_once` fills a temp directory and renames it into place, the way the store path does. Extracting straight into the final entry path leaves a PARTIAL entry on every failure below the download. `restore` reads any non-empty entry as a hit. Cargo then gets a package whose rlib never arrived, and each dependent reports `E0463: can't find crate` for a crate the same build is compiling. `ci/pkg-remote-test.sh` guards this. It stubs a binpazer that writes the rmeta and fails on the rlib, then asserts the store is empty.
- **The leg without the wrapper is what says whose bug it is.** Read `nocache-j3` against `Build & test` on one commit. Green on the first and red on the second names the cache. It is the only thing that differs between them.

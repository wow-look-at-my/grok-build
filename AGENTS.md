...
- Its uploads go through `buildhost-upload-artifact`, one step per platform. **The buildhost CLI cannot run on an ubuntu runner.** buildhost publishes it as ONE Actually Portable Executable. So `?os=linux&arch=amd64` serves the same `MZqFpD` file a Mac gets. This runner registers a binfmt handler for that magic. It answers `run-detectors: unable to find an interpreter`. The deb does not help. Its `/usr/bin/buildhost` shim still execs the APE. `binpazer` is a plain ELF, which is why the download of that one works here.
- The action is TypeScript. It mints its own OIDC bearer from `ACTIONS_ID_TOKEN_REQUEST_URL`, and it chunks a body past the edge's cap the way the CLI does. So the job needs no bearer step and no CLI.
- Composite action steps cannot overlap. So the uploads are sequential now. Each names the release the `release` job already created. The `buildhost-publish-release` step publishes one time at the end.
...
- A slot swaps the whole SAMPLER, never the model id alone. `resolve_slot_sampler` builds the config from the CATALOG. The backend, the window and the credentials belong to the model the slot names. Writing one model's id onto the session's config sends that id to the session model's endpoint. Compaction, the laziness classifier and the permission classifier were the first to do it correctly.
- `prepare_side_call` is that resolution for recap, turn summary, `/btw` and `/todo`. It hands back the slot's client, its window and its model together. Reading the slot's model beside a client the caller resolved itself is the defect this shape prevents.
- `resolve_sampler_for_model` is the same resolution for a model no slot named. `[memory] flush_model` is the narrower key that still wins over its slot. It needs the catalog too.
- A slot that resolves to nothing keeps the session's client AND the session's model. That covers an unset slot, a slot naming the session's own model, and a pin this session cannot reach.
...
- `build-test` restores ONE `actions/cache` entry. The entry holds the registry-dependency artifacts and the cargo registry. Measured on run 35459915405: registry crates are 1909 of that job's 4161 compile CPU-seconds.
...
- Only `master` WRITES an entry. `cached-run` gates the save on `save-on`, which defaults to `default-branch`. Nothing in this repository sets it, so every branch here reuses and writes nothing.
- Before that gate existed, each ref that missed saved its own copy. This repository then held the 1.7 GB `rustdeps-dev` entry several times over. The copies were master's, one orphaned by a key-scheme bump, and one a branch wrote when its restore raced master's save.
- Reading is not gated, and never was. GitHub states the rule in "Restrictions for accessing a cache". A run restores a cache created in the current branch or in the default branch. A run cannot restore one created for a child branch or a sibling branch. So a branch keeps every hit it had.
...
- A declared provider makes the grok.com sign-in OPTIONAL. `config::any_provider_has_own_credentials` asks the PROVIDER block the `has_own_credentials` question the catalog is asked of a model (through `provider_probe_entry`, the same `entry_for_provider_model` merge), and `should_advertise_xai_api_key*` takes it as BYOK. The catalog cannot answer it here: autodetection runs on a `spawn_local` off `initialize` and is never awaited, so a user who declared a provider and wrote no `[model.<id>]` block reached the auth-method build with an empty catalog and was sent to the login screen for an endpoint grok.com never sees. The `xai.api_key` authenticate arm and the cached-token fallthrough read the same signal, so neither answers "no credentials" for a provider-only config.
...
- **The green dot.** `ModelInfo.loaded_in_vram` is `Option<bool>` and rides `meta.loadedInVram` to the picker. The three answers are distinct: a remote model writes no key and draws no dot, a loaded local model draws `theme.accent_success`, an unloaded one draws `theme.gray_dim`. Collapsing the absent case into `false` puts a cold dot beside every cloud model in the list. The dot's column is reserved for the WHOLE list (`dot_column_w`), not per row: a per-row reservation starts the label at a different column on a dotted row than on a plain one, and a picker holding both local and cloud models reads as two ragged lists.
- `loaded_in_vram` is never persisted. It describes the runtime at one instant, and a replayed value claims a load that ended.
- The dot is POLLED, not painted once. A local runtime loads a model on its first request and unloads it on an idle TTL, so residency read at catalog build is wrong within minutes. `refresh_local_residency` re-reads only the residency (`/api/ps`, or LM Studio's listing) on `RESIDENCY_POLL_INTERVAL`, and `apply_local_residency` pushes a models-updated ONLY when a dot actually changed — a push per tick redraws the picker of every connected client for nothing. An unreachable runtime leaves the dots as they were, because reporting every model cold would claim an unload nobody observed.
- **`extra_body` is the escape hatch, and the typed request structs are why.** `ChatCompletionRequest` and its siblings are closed, so a per-deployment setting only one target understands has nowhere else to go: LM Studio's `ttl`, Ollama's `keep_alive`, `truncate` and `options.num_ctx` are each one of those. It rides `SamplerConfig.extra_body` (per client, since it is per-model config) and merges into the serialized body through `merge_extra_body`, AFTER the builder ran, so it can never displace a field the builder decided. A dotted key addresses a nested object, because TOML cannot spell a nested table inline beside scalar siblings.
- **Ollama's compat endpoint cannot carry `num_ctx` at all**, so `extra_body` alone does not fix the window there: the compat handler's request struct has no `options` member and Go discards unknown fields. Preloading with an explicit `num_ctx` and then using `/v1/chat/completions` does not work either — `runnerRef.needsReload` compares `Runner` options, and an auto-`num_ctx` request against an explicitly-loaded runner triggers a reload that throws away both the preload and the warm KV cache. `api_backend = "ollama"` is the only way to pin it.
- `ApiBackend::Ollama` posts `/api/chat`, whose paths are SIBLINGS of the compat endpoint at the host root. One provider block names one base URL for both, so the client strips `/v1` when it builds its endpoint template (`native_host_root`).
- The response is NDJSON, not SSE: one whole JSON object per line, no event framing and no `[DONE]`. `ndjson_chunk_stream` reassembles lines across transport chunk boundaries and treats what is left in the buffer at end of stream as a line, because the last one usually arrives without a newline.
- A tool call arrives WHOLE on one line, with its arguments as an OBJECT rather than a string of fragments. `stream_ollama` emits it as a single `ToolCallDelta` carrying the complete arguments — a consumer that waits for a second fragment waits forever — and converts the object to the JSON string every other backend carries. A call Ollama gave no id still gets one, or two calls to the same tool in one message cannot be told apart by their results.
- A line carrying only `error` is a failed run. Ending the stream quietly there hands the turn a truncated answer as a finished one, so both the L2 transform and the compaction arm fail on it.
- Ollama's thinking is unsigned plain text, so its reasoning item carries no `encrypted_content` and `ThinkingReplayPlan` always replays it natively. A reasoning item and the assistant message it belongs to are two items here and ONE message on that wire; sending them apart is two consecutive assistant turns in the model's own template.
- **Pricing.** A local model charges nothing and is in no catalog, so the modelinfo lookup can only 404 — once per TTL, forever. `pricing_lookup_enabled = false` covers a CONFIGURED model. A DISCOVERED one is not in config at all, and `resolve_configured_pricing` rebuilds the catalog from config alone, so discovery also registers its ids with `model_pricing::suppress_lookup_for`.

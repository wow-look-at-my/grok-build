...
mod tests {
    ...
    /// Every OTLP export fails with "otlp export disabled" before any network
    /// I/O. A real send to the unroutable port gives a different error.
    #[test]
    fn send_bytes_returns_disabled_error() {
        ...
        let err = runtime
            .block_on(client.send_bytes(request))

...
// ---------------------------------------------------------------------------
// User-managed OpenAI-compatible endpoint. All fields are optimistic mirrors;
// they take effect for new sessions after the config write completes.
// ---------------------------------------------------------------------------

fn compatible_restart_toast(label: &str, value: &str) -> String {
    format!("\u{2713} {label}: {value} (new session to apply)")
}

fn compatible_profile_from_snapshot(
    snapshot: &crate::settings::OpenAiCompatibleSnapshot,
) -> xai_grok_shell::agent::config::OpenAiCompatibleConfig {
    xai_grok_shell::agent::config::OpenAiCompatibleConfig {
        enabled: snapshot.enabled,
        base_url: snapshot.base_url.clone(),
        model: snapshot.model.clone(),
        api_backend: if snapshot.api_backend == "responses" {
...
pub(super) fn set_openai_compatible_enabled_inner(app: &mut AppView, value: bool) {
    app.openai_compatible.enabled = value;
}

pub(in crate::app::dispatch) fn set_openai_compatible_enabled(
    app: &mut AppView,
    value: bool,
) -> Vec<Effect> {
    if value {
        let mut candidate = compatible_profile_from_snapshot(&app.openai_compatible);
        candidate.enabled = true;
        if let Some(error) = candidate.validation_error() {
            app.show_toast(&format!(
                "\u{2717} Configure the compatible endpoint first: {error}"
            ));
            return vec![];
        }
    }
    let previous = app.openai_compatible.enabled;
    if previous == value {
        return vec![];
    }
...
pub(super) fn set_openai_compatible_base_url_inner(app: &mut AppView, value: String) {
    app.openai_compatible.base_url = value;
}

pub(in crate::app::dispatch) fn set_openai_compatible_base_url(
    app: &mut AppView,
    value: String,
) -> Vec<Effect> {
    let value = value.trim().trim_end_matches('/').to_owned();
    let previous = app.openai_compatible.base_url.clone();
    if previous == value {
        return vec![];
    }
    set_openai_compatible_base_url_inner(app, value.clone());
    refresh_open_settings_modals(app);
    app.show_toast(&compatible_restart_toast("Compatible base URL", &value));
    vec![Effect::PersistSetting {
...
pub(super) fn set_openai_compatible_model_inner(app: &mut AppView, value: String) {
    app.openai_compatible.model = value;
}

pub(in crate::app::dispatch) fn set_openai_compatible_model(
    app: &mut AppView,
    value: String,
) -> Vec<Effect> {
    let value = value.trim().to_owned();
    let previous = app.openai_compatible.model.clone();
    if previous == value {
        return vec![];
    }
    set_openai_compatible_model_inner(app, value.clone());
    refresh_open_settings_modals(app);
    app.show_toast(&compatible_restart_toast("Compatible model", &value));
    vec![Effect::PersistSetting {
...
pub(super) fn set_openai_compatible_api_backend_inner(app: &mut AppView, value: &str) {
    app.openai_compatible.api_backend = value.to_owned();
}

pub(in crate::app::dispatch) fn set_openai_compatible_api_backend(
    app: &mut AppView,
    value: String,
) -> Vec<Effect> {
    ...
    refresh_open_settings_modals(app);
    app.show_toast(&compatible_restart_toast("Compatible API", value));
    vec![Effect::PersistSetting {
...
pub(super) fn set_openai_compatible_make_default_inner(app: &mut AppView, value: bool) {
    app.openai_compatible.make_default = value;
}

pub(in crate::app::dispatch) fn set_openai_compatible_make_default(
    app: &mut AppView,
    value: bool,
) -> Vec<Effect> {
    let previous = app.openai_compatible.make_default;
    if previous == value {
        return vec![];
    }
    ...
    refresh_open_settings_modals(app);
    app.show_toast(&compatible_restart_toast(
        "Use compatible endpoint by default",
        if value { "on" } else { "off" },
    ));
    vec![Effect::PersistSetting {
...
pub(super) fn set_openai_compatible_api_key_configured_inner(app: &mut AppView, value: bool) {
    app.openai_compatible.api_key_configured = value;
}

pub(in crate::app::dispatch) fn set_openai_compatible_api_key(
    app: &mut AppView,
    secret: crate::app::actions::SecretString,
) -> Vec<Effect> {
    let previous = app.openai_compatible.api_key_configured;
    let configured = !secret.expose_secret().trim().is_empty()
        || std::env::var("OPENAI_API_KEY")
            .ok()
            .is_some_and(|value| !value.trim().is_empty());
    set_openai_compatible_api_key_configured_inner(app, configured);
    refresh_open_settings_modals(app);
    app.show_toast(&compatible_restart_toast(
        "Compatible API key",
        if configured { "configured" } else { "cleared" },
    ));
    vec![Effect::PersistOpenAiCompatibleApiKey {
        secret,
        rollback_configured: previous,
    }]
}

...
/// Past Always-Approve the ring steps through the agent-identity stops
/// (Orchestrator, Explore) and then closes on Plan; see `shift_tab_ring_agent_index`.
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    // Orchestrator and Explore say WHO the agent is, not how much it asks, so
    // the permission mode rides through untouched. Revoking always-approve
    // here silently re-armed the permission prompt on an unattended session
    // whose whole point was not having one. With no stops configured, the
    // choice list below wraps as usual.
    if in_yolo
        && mode == SessionMode::Default
        && let Some(&first) = xai_grok_agent::config::BuiltinAgentName::shift_tab_variants().first()
    {
        let label = shift_tab_agent_label(first);
        agent.shift_tab_base_agent = Some(
            agent
                .session_agent_name
                .clone()
                .unwrap_or_else(|| "grok-build".to_string()),
        );
        agent.shift_tab_ring_agent_index = Some(0);
        agent.show_mode_switch_banner(label);
        refresh_open_settings_modals(app);

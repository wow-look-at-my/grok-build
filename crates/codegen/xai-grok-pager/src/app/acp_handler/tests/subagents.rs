#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn subagent_spawned_resolves_model_id_to_friendly_name() {
        ...
        // The shared spawn fixture, but with a raw model *id* set.
        let mut spawned = test_subagent_spawned("sess-parent", child_sid);
        let XaiSessionUpdate::SubagentSpawned { model, .. } = &mut spawned else {
            unreachable!();
        };
        *model = Some("agent-host".to_string());
        ...
        // The attempt's model must be the resolved friendly name, not the raw id.
        assert_eq!(
            info.attempt.model.as_deref(),
        ...
        let entry_id = info
            .attempt
            .scrollback_entry_id
            .expect("spawn must stash block id");
        let entry = agent.scrollback.get_by_id(entry_id).unwrap();
        let RenderBlock::Subagent(sb) = &entry.block else {
            panic!("SubagentSpawned must push a SubagentBlock to parent scrollback");
        };

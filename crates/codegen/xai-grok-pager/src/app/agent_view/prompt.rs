        ...
        if let Some(action_id) = registry.lookup(key, When::PromptFocused) {
            match action_id {
                ActionId::SendPrompt => {
                    ...
                    // Empty composer + mid-turn queue: interrupt the turn with the queue
                        ...
                        && !slash_accepted_send
                    {
                        if matches!(self.prompt_mode, PromptMode::Normal)
                            ...
                            && let Some(outcome) = if self.session.state.is_turn_running() {
                                self.try_interrupt_with_queued_from_prompt()
                            } else {
                                self.try_send_now_queued_from_prompt()
                            }
                        {
                            return outcome;
                        }
                ...
                ActionId::InterjectPrompt => {
                    ...
                    } else if let Some(outcome) = if self.session.state.is_turn_running() {
                        self.try_interrupt_with_queued_from_prompt()
                    } else {
                        self.try_send_now_queued_from_prompt()
                    } {

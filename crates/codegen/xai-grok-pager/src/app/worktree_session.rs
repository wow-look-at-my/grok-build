...
pub(crate) fn resume_worktree_params(
    ...
    if let Some(obj) = params.as_object_mut() {
        ...
        if include_agents {
            obj.insert("includeAgents".into(), serde_json::Value::Bool(true));
        }
    }
    params
}
...
pub(crate) async fn resume_session_into_worktree(
    ...
    let params = resume_worktree_params(source_cwd, spec, session_id, restore_code, include_agents);
    let req = acp::ExtRequest::new(
        RESUME_METHOD,
        serde_json::value::to_raw_value(&params)
...
mod tests {
    ...
    fn resume_params_omit_restore_code_when_unset() {
        ...
        let p = resume_worktree_params(Path::new("/repo"), &spec, "sid", None, false);
        ...
        let p = resume_worktree_params(Path::new("/repo"), &spec, "sid", Some(true), false);

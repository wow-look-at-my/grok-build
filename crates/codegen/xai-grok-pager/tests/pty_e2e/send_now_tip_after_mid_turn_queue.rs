...
/// After queuing a follow-up mid-turn, the ephemeral tip advertises the
/// interrupt (`… to interrupt & send`). Opt into contextual hints explicitly so
/// the tip cannot be soft-disabled by remote defaults in CI.

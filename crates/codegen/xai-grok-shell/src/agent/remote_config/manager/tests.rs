...
fn resolve_context_window_returns_each_models_own_window_from_multi_model_listing() {
    ...
    assert_eq!(
        super::super::resolve_context_window("openai-foo", &listing).get(),
        128_000
    );
    assert_eq!(
        super::super::resolve_context_window("openai-bar", &listing).get(),
        200_000
    );
    assert_eq!(
        super::super::resolve_context_window("openai-baz", &listing).get(),
        1_000_000
    );
}
...
fn resolve_context_window_matches_by_routing_slug_even_when_key_differs() {
    ...
    assert_eq!(
        super::super::resolve_context_window("grok-4", &listing).get(),
        256_000,
...
fn resolve_context_window_absent_slug_falls_back_to_documented_default() {
    ...
    assert_eq!(
        super::super::resolve_context_window("totally-unknown-model", &listing).get(),
        default,
...
fn resolve_context_window_empty_listing_falls_back_to_documented_default() {
    let default = crate::remote::DEFAULT_CONTEXT_WINDOW;
    assert_eq!(
        super::super::resolve_context_window("absent-model", &IndexMap::new()).get(),
        default,
...
fn resolve_context_window_drives_auto_compaction_threshold() {
    ...
    let resolved = super::super::resolve_context_window("big", &listing);
    assert_eq!(resolved.get(), 1_000_000);
    // And never a max/first match across the listing.
    assert_eq!(
        super::super::resolve_context_window("small", &listing).get(),
        32_000
    );
...
fn production_resolve_model_list_backfills_window_per_slugs_into_compaction() {
    ...
    let resolved_cw = super::super::resolve_context_window("big-preview", &full_listing);

...
use xai_grok_sampling_types::{
    ...
    endpoint_host, reasoning_effort_meta_value, reasoning_efforts_meta_value,
};
...
pub(crate) fn resolve_model_list(
    ...
    }
    {
        ...
        for (key, entry) in resolved.iter_mut() {
            if entry.info.context_window.get() == default_cw {
                let resolved_cw = crate::agent::remote_config::resolve_context_window(
                    &entry.info.model,
                    &cw_sources,
                );
            ...
            if still_default && entry.own_credential().is_some() {
                ...
                if let Some(cw) = crate::agent::remote_config::resolve_context_window_from_provider(
                    &entry.info.model,
                    &base_url,
                    api_key.as_deref(),
                ) {

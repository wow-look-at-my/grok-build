...
fn fallback_notices(rx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>) -> Vec<XaiUpdate> {
	let mut out = Vec::new();
...
async fn a_goal_role_runs_on_a_model_outside_allowed_models() {
	...
	local
		.run_until(async {
			...
			actor
				.models_manager
				.insert_test_entry("outside-allowlist", catalog_entry("outside-allowlist", false));
...
async fn an_unknown_goal_role_model_falls_back_and_says_how_to_fix_it() {
	...
	local
		.run_until(async {
			...
			assert_eq!(resolved.model, None, "an unknown model runs on the session model");

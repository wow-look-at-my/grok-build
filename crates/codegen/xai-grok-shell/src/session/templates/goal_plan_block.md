...
- The plan's steps are ALREADY on your todo list — the goal planner added one item per step, in order, so do not re-read this plan to populate it. Work them in order and keep each item's status current through `{TODO_TOOL}`.
...
- Before claiming completion, run the plan's `## Verification plan` yourself and confirm its observations hold. Checking is not doing: a step reads back what you built, and never authorizes work the objective did not ask for. Commit real tests that drive the shipped code in-repo, and RUN them. The harness records each run for the verifier, so save no proof files. Fix any missing observation before calling the goal complete.

...
use super::super::*;
use super::{
    AutoCompactTriggerInfo, COMPACT_FAILURE_DETAIL_LIMIT, SuppressReason, compose_compact_failure,
};
...
/// `other` and `schema` clear next turn, `credit_block` holds until a successful model call, `size` is sticky until a full reset (success/rewind/model switch).
    ...
    local
        .run_until(async {
            ...
            for per_turn in [SuppressReason::Other, SuppressReason::Schema] {
                actor
                    .suppress_auto_compaction(per_turn, "", 1_000, 200_000)
                    .await;
                assert_eq!(
...
fn suppression_notification_message_is_reason_specific() {
    ...
    // Every canned reason keeps what the provider said after its advice.
    assert_eq!(
        msg(SuppressReason::CreditBlock, detail),
        "out of credits or over your spending limit. Add credits and retry. — API error (status 500 Internal Server Error)"
    );
    assert_eq!(
        msg(SuppressReason::Auth, detail),
        "authentication problem — re-authenticate using /login and retry. — API error (status 500 Internal Server Error)"
    );
    assert_eq!(
        msg(SuppressReason::Size, detail),
        "this conversation is too large to compact. — API error (status 500 Internal Server Error)"
    );
    assert_eq!(
        msg(SuppressReason::Schema, detail),
        "the model rejected the summarization request as malformed. — API error (status 500 Internal Server Error)"
    );
    assert_eq!(
        msg(SuppressReason::Size, ""),

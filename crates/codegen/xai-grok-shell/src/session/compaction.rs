...
impl SessionActor {
    ...
    /// Every reason carries the provider's own `detail` after its advice.
    ...
    /// Text for the suppression notification: canned advice per reason, then what the provider said.
    /// The advice names a class of failure; only the detail names the field, the item or the id to fix.
    ...
    fn suppress_notification_message(reason: SuppressReason, detail: &str) -> String {
        let advice = match reason {
            ...
            SuppressReason::Schema => "the model rejected the summarization request as malformed.",
            SuppressReason::Other => return Self::failure_with_retry_guidance(detail),
        };
        compose_compact_failure(advice, &Self::user_facing_compact_error(detail))
    }
    ...
    async fn run_compact_inner(
        ...
        let user_context = self.merge_goal_compaction_user_context(user_context).await;

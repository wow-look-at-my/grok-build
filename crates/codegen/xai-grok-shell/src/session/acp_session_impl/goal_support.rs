...
impl SessionActor {
        ...
        loop {
            ...
            match outcome {
                crate::session::goal_planner::GoalPlannerOutcome::Planned {
                    ...
                    // The plan is published. Populate the session's todo list
                    // from it HERE, before the goal-start reminder is rendered,
                    // so the implementing turn opens with the plan's steps
                    // already on its list instead of re-reading the plan to
                    // transcribe them (and risking losing one doing it).
                    let published_plan = self.goal_tracker.lock().plan_path();
                    self.seed_goal_todos_from_plan(&goal_id, &published_plan, &planner_todos)
                        .await;
    ...
    /// Populate the session's todo list from the plan the goal planner just
    /// published.
    ///
    /// The plan names its work twice over: the planner calls `todo_write` on
    /// its OWN list while it works (read back out of the child when it finished
    /// — `planner_todos`), and it writes the plan body. `planner_todos` wins;
    /// when the planner named nothing the plan body is parsed
    /// ([`plan_todo_items_from_path`]). So the list is populated on every
    /// publish, whether or not the planner reached for the tool — the main
    /// session never transcribes the plan itself.
    ///
    ...
    /// retry, a resume, or a re-entry cannot add a second copy of a step.
    ...
    /// over the same plan; the publish path itself calls it exactly once.
    pub(super) async fn seed_goal_todos_from_plan(
        &self,
        goal_id: &str,
        plan_path: &Path,
        ...
        let claimed = {
            ...
            tracker
                ...
                .filter(|goal| {
                    goal.goal_id == goal_id
                        && goal.status == crate::session::goal_tracker::GoalStatus::Active
                        && goal.plan_file.as_deref() == Some(plan_path)
        ...
        let items: Vec<String> = if planner_todos.is_empty() {
            crate::session::goal_next_step::plan_todo_items_from_path(plan_path)
        } else {
        ...
        if items.is_empty() {
            tracing::debug!(
                ...
                "goal planner: plan named no steps; nothing to seed"
            );
            return 0;
        }
            ...
            Err(err) => {
                tracing::warn!(
                    ...
                    "goal planner: session has no append-capable todo tool; \
                     the plan's steps were not seeded"
                );
                return 0;
            }
        };

        // Dedupe against what is already on the list: a step the user or the
        // main agent already wrote is the same step, and a plan that repeats
        ...
        let fresh: Vec<String> = items
            ...
            .collect();
        if fresh.is_empty() {
            tracing::debug!("goal planner: every plan step is already on the todo list");
            return 0;
        }
            ...
            .await
        {
            Ok(_) => {
                tracing::info!(
                    count = fresh.len(),
                    "goal planner: seeded the session todo list from the plan"
                );
                fresh.len()
            }
            Err(err) => {

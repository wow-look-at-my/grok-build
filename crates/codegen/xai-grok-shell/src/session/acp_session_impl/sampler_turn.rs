...
impl SessionActor {
        ...
        match collected.result {
            ...
            Err(rich_err) => {
                *self.in_flight_sampler_request_id.lock() = None;
                ...
                if outcome != StreamDrainOutcome::Revoked {
                    // ASAP injection: the `SessionCommand::Interject` handler cancelled this request so
                    // the turn loop can drain the interjection and resubmit at once. The streamed text is
                    // kept, so the resubmitted request sees the partial assistant turn and then the user
                    // interjection. The "request cancelled" error is not a real failure.
                    if self
                    ...
                    // With length salvage off, a response cut at the output token cap arrives here as a
                    // fatal `MaxTokensTruncation`. Keep the streamed text and resubmit with a continue
                    // reminder instead of failing the turn. Length salvage, when on, owns this case.
    ...
    async fn recover_from_sampling_failure(
            ...
            .await?
        {
            ...
            SamplerFailureRecovery::ReduceAndResubmit => Ok(SamplerTurnOutcome::ReduceAndResubmit),
    ...
    pub(crate) fn record_response_token_usage(
        ...
        if let Some(ref u) = response.usage {
            ...
            let cost_ticks = response.cost_usd_ticks.or_else(|| {
                ...
                let pricing = crate::agent::model_pricing::resolve(model_id);

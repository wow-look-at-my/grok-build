...
impl SessionActor {
    ...
    /// The wait is bounded by [`Self::mcp_init_wait_budget`]: a server that never settles must not hang the
    /// prompt that waits on it. Proceeding without that server is better than a session that answers nothing.
    pub(super) async fn wait_for_mcp_initialized(&self) {
        let budget = self.mcp_init_wait_budget();
        if tokio::time::timeout(budget, self.wait_for_mcp_initialized_unbounded())
            .await
            .is_err()
        {
    ...
    /// How long any caller may wait on MCP initialization: twice the configured
    /// startup timeout plus 5s, and a grace margin on top.

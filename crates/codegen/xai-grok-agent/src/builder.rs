...
impl AgentBuilder {
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.write_file_enabled && !has_write_tool && has_edit_tool && !read_only {
                tool_config
                    .tools
        ...
        if !self.subagents_enabled {
            ...
            if subagents.is_empty() {
                ...
                .find(|tc| tc.id == task_tool_id)
            {
                task_tc.description_override = Some(task_tool_description(
...
/// The Task tool description: the shared text, the model guidance, then the
/// `[subagents] usage_frequency` note when one is configured.
pub(crate) fn task_tool_description(
...
mod tests {
    ...
    #[test]
    fn task_tool_description_default_frequency_has_no_usage_note() {
        let desc = task_tool_description(
            TaskModelSelection::Selectable,
            &[],
            xai_tool_types::AgentUsageFrequency::Default,
    ...
    #[test]
    fn task_tool_description_explicit_only_tells_model_not_to_use_it_unasked() {
        let desc = task_tool_description(
            TaskModelSelection::Selectable,
            &[],
    ...
    #[test]
    fn task_tool_description_very_often_tells_model_to_default_to_delegating() {
        let desc = task_tool_description(
            TaskModelSelection::Selectable,
            &[],

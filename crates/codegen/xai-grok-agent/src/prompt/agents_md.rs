...
async fn read_agents_config_with_roots(
    ...
    for candidate in candidates {
        ...
        files.push(config_file(candidate.path, content, source));
        files.extend(
            imported
                .into_iter()
                .map(|file| config_file(file.path, file.content, source)),
        );
    }
    files
}

/// An imported file carries the source of the file that named it.
fn config_file(path: PathBuf, content: String, source: InstructionSource) -> AgentConfigFile {
    let file_name = path
        .file_name()

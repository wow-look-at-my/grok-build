...
impl ImageStripReason {
    fn placeholder(self) -> &'static str {
        match self {
            Self::PayloadRejected => IMAGE_STRIP_PLACEHOLDER,
...
impl ConversationRequest {
    /// Strip every image; returns the stripped URLs. `reason` picks the user-image placeholder.
    pub fn strip_images(&mut self, reason: ImageStripReason) -> Vec<Arc<str>> {
        strip_images_where(&mut self.items, reason.placeholder(), |_| true)
    }
...
pub fn strip_images_by_url(items: &mut [ConversationItem], urls: &[Arc<str>]) -> usize {
    strip_images_where(items, IMAGE_STRIP_PLACEHOLDER, |url| {
        urls.iter().any(|u| u.as_ref() == url)
    })
    .len()
}
...
mod tests {
    ...
    fn test_strip_images_placeholder_names_the_reason() {
        let placeholder_for = |reason| {
            ...
            user.add_image("data:image/png;base64,abc123".to_string());
            req.items.push(user);
            assert_eq!(req.strip_images(reason).len(), 1);
            match &req.items[0] {
        ...
        assert!(too_large.contains("could not process"), "got: {too_large}");
        ...
        assert!(
            !no_vision.contains("could not process"),

...
struct FallbackSettings {
    ...
    messages_cost_usd_ticks: Option<i64>,
...
impl InferenceRoute {
    ...
    pub(crate) fn set_messages_cost_usd_ticks(&self, ticks: Option<i64>) {
        self.fallback.write().unwrap().messages_cost_usd_ticks = ticks;
    }
    ...
    async fn serve(self, endpoint: InferenceEndpoint, headers: HeaderMap, body: Value) -> Response {
        ...
        let events = match (endpoint, &mode) {
            ...
            (InferenceEndpoint::Messages, ResponseMode::Echo | ResponseMode::Fixed(_)) => {
                sse::messages_api_script_with_cost(
                    &text,
                    model,
                    &settings.messages_stop_reason,
                    settings.messages_cost_usd_ticks,
                )
            }
        };

...
pub fn messages_api_events_with_cost(
    ...
    scripted_to_axum(messages_api_script_with_cost(
        text,
        model,
        stop_reason,
        cost_usd_ticks,
...
pub fn messages_api_script(text: &str, model: &str, stop_reason: &str) -> Vec<SseEvent> {
    messages_api_script_with_cost(text, model, stop_reason, None)
}

/// [`messages_api_events_with_cost`] as [`SseEvent`]s.
pub fn messages_api_script_with_cost(
    text: &str,
    model: &str,
    stop_reason: &str,
    cost_usd_ticks: Option<i64>,
